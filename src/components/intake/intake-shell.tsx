    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 py-10 lg:flex-row lg:px-10 lg:py-12">
        <aside className="w-full max-w-xl space-y-6 lg:sticky lg:top-10 lg:w-88 lg:self-start">
          ...
          <div className="space-y-3 rounded-[18px] border border-white/10 bg-secondary/55 px-6 py-5 shadow-[0_14px_34px_rgba(1,6,20,0.3)]">
                ...
                <li key={step.id}>
                  <div
                    ...
                    className={cn(
                      ...
                      isActive && "border-white/14 bg-secondary/70 shadow-[0_8px_24px_rgba(1,6,20,0.24)]",
                    )}
                  >
                    <span
                      className={cn(
                        ...
                        !isActive && !isComplete && "border-white/12 text-white/56",
                      )}
                    >
                      {index + 1}
                    </span>
                    <div className="space-y-1">
                      <p className="font-semibold text-white/92">{step.label}</p>
        ...
        <main className="flex-1 space-y-6">
          <div className="space-y-3">
            <p aria-live="polite" className="text-sm font-semibold text-white/56">
              {stepSummary}
            </p>
                ...
                <fieldset className="space-y-4">
                  ...
                  <RadioGroup defaultValue="extension" className="gap-3">
                    {PROJECT_TYPES.map((option) => (
                      <label
                        key={option.value}
                        className="flex items-center gap-3 rounded-[14px] border border-white/10 bg-secondary/45 px-4 py-3 text-white/86 transition-all duration-200 ease-out hover:border-white/20 hover:bg-secondary/65"
                      >
            ...
            {currentStep.id === "documents" ? (
              <section className="space-y-4">
                ...
                <ul className="grid gap-3 text-sm text-white/70 sm:grid-cols-2">
                  <li className="rounded-[14px] border border-white/10 bg-secondary/45 px-4 py-3">
